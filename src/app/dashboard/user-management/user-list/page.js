  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/dashboard/user-management/add-user" className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 font-medium">
          + Add User
        </Link>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {["admin","manager","cashier"].map((role) => (
          <div key={role} className={`rounded-xl p-4 border ${role === "admin" ? "bg-red-50 border-red-100" : role === "manager" ? "bg-blue-50 border-blue-100" : "bg-green-50 border-green-100"}`}>
            <p className="text-xs text-gray-500 capitalize">{role}s</p>
            <p className={`text-2xl font-bold ${role === "admin" ? "text-red-700" : role === "manager" ? "text-blue-700" : "text-green-700"}`}>
      ...
      <div className="bg-white rounded-xl shadow p-4 mb-4 flex flex-wrap gap-3 items-end">
        <form onSubmit={(e) => { e.preventDefault(); fetchUsers(); }} className="flex gap-2">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search users..." className="border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-48" />
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-96">
            ...
            <div className="flex gap-3">
              <button onClick={() => handlePasswordChange(changingPassword)} className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700">
                Update Password
              </button>
              <button onClick={() => setChangingPassword(null)} className="flex-1 bg-gray-100 text-gray-700 px-4 py-2 rounded-lg text-sm hover:bg-gray-200">
                Cancel
              </button>
            </div>
          </div>
