...
export default function CreateQuotationPage() {
  ...
  const [custInfo,   setCustInfo]   = useState({ name: "", mobile: "" });
  const [scanVal,    setScanVal]    = useState("");
  const [validUntil, setValidUntil] = useState("");
  const [items,      setItems]      = useState(() => [newRow()]);
  const [pay,        setPay]        = useState({ overallDiscount: "0", taxAmount: "0", notes: "" });
  ...
  const selectCustomer = (c) => {
    ...
    setCustInfo({ name: c.name, mobile: c.phone || c.mobile || "" });
    setCustOpen(false);
  };
  ...
  return (
    <div className="min-h-full flex flex-col bg-white pb-14">
      ...
      <div className="bg-white border-b border-gray-200 px-4 pt-3 pb-3">
        <h1 className="text-sm font-bold mb-2" style={{color:"#1E3A8A"}}>Quotation</h1>
      ...
      <div className="fixed bottom-0 left-0 right-0 lg:left-64 flex h-14 z-50">
        ...
        <button type="button" onClick={handleSave} disabled={saving}
          className="w-56 flex items-center justify-center bg-[#2563EB] hover:bg-blue-700 text-white text-sm font-bold uppercase tracking-wider transition-colors disabled:opacity-60">
