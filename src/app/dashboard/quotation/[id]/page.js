...
function QuotationViewContent() {
  ...
  useEffect(() => {
    api.getQuotation(id)
      .then((r) => { setQuotation(r.data); setLoading(false); })
    ...
    <div>
      ...
      <div className="no-print flex items-center justify-between gap-3 flex-wrap mb-5">
        ...
        <div className="flex gap-2">
          <Link href={`/dashboard/quotation/${id}/edit`}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-8 max-w-2xl mx-auto shadow-sm">
        ...
        <div className="flex justify-between items-start mb-6 pb-4 border-b border-gray-200">
          <div>
            <h1 className="text-2xl font-bold text-[#1E3A8A]">QUOTATION</h1>
            <p className="text-gray-500 text-sm mt-0.5 font-mono">{quotation.quotationNumber}</p>
          </div>
          <div className="text-right text-sm">
            <p className="text-gray-600">{new Date(quotation.createdAt).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</p>
            <span className={`inline-block mt-1 px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${STATUS_COLORS[quotation.status] || "bg-gray-100 text-gray-600"}`}>
              {quotation.status}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6 text-sm">
          <div>
            ...
            {quotation.customer?.phone && <p className="text-gray-500 text-xs mt-0.5">{quotation.customer.phone}</p>}
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Valid Until</p>
            <p className="font-medium text-gray-800">
              {quotation.validUntil ? new Date(quotation.validUntil).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : "—"}
            </p>
          </div>
        </div>
          ...
          <tbody>
            {quotation.items.map((it, i) => (
              <tr key={i} className="border-b border-gray-100">
                ...
                <td className="py-2.5 text-right text-gray-700">৳{(it.unitPrice || 0).toFixed(2)}</td>
                <td className="py-2.5 text-right text-gray-500">৳{(it.discount || 0).toFixed(2)}</td>
                <td className="py-2.5 text-right font-semibold text-gray-800">৳{(it.total || 0).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="flex justify-end">
          <div className="w-full sm:w-56 text-sm space-y-1.5">
            ...
            {quotation.discountAmount > 0 && (
              <div className="flex justify-between text-gray-600">
                <span>Discount</span>
                <span className="text-red-500">-৳{quotation.discountAmount.toFixed(2)}</span>
              </div>
            )}
