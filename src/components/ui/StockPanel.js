...
function StatusBadge({ stock, alertQty }) {
  if (stock === 0)
    return <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-red-100 text-red-600">Out of Stock</span>;
  if (stock <= alertQty)
    return <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-orange-100 text-orange-600">Low Stock</span>;
  return <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-green-100 text-green-600">In Stock</span>;
}
...
function StatCard({ label, value, color }) {
  ...
  return (
    <div className={`${c.bg} border ${c.border} rounded-xl p-4 flex items-center gap-3`}>
      <div className={`w-10 h-10 rounded-full ${c.dot} opacity-20 flex items-center justify-center`}>
        <span className={`w-3 h-3 rounded-full ${c.dot} opacity-100 block`} />
      </div>
      <div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-[#1E3A8A]">Stock Overview</h2>
        ...
        <button
          onClick={onClose}
          className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 hover:border-gray-300 transition-colors shadow-sm"
        >
      ...
      {/* ── Search + Filter ── */}
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-4 flex flex-col sm:flex-row gap-3">
